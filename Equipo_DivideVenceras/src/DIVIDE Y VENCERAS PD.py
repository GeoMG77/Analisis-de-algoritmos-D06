...
# --- Algoritmo de Decodificación (Programación Dinámica) ---
def decodificar_memo(cad: str) -> int:
    tam = len(cad)
    memo = {}
...
class DecodificadorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Decodificador de Mensajes (Programación Dinámica)")
        self.root.geometry("600x650")
        
        self.datos_n = []
        self.datos_tiempos = []
        
        self.crear_widgets_superiores()
        self.crear_grafica()

    def crear_widgets_superiores(self):
    ...
    def crear_grafica(self):
        ...
        self.ax.set_title("Rendimiento en Tiempo Real (Prog. Dinámica)")
        self.ax.set_xlabel("Longitud de la cadena (n)")
        self.ax.set_ylabel("Tiempo (segundos) - Escala Logarítmica")
        self.ax.grid(True)
        self.ax.set_yscale('log')
        self.line, = self.ax.plot(self.datos_n, self.datos_tiempos, 'go-') 
    ...
    def iniciar_analisis(self):
        ...
        if not cad.isdigit() or len(cad) == 0:
            self.txt_resultados.insert(tk.END, "Error: Ingresa una cadena numérica válida.")
            return
    ...
    def hilo_calcular(self, cad: str):
        try:
            ...
            total = decodificar_memo(cad)
            end_time = time.perf_counter()
            duracion = end_time - start_time

            if n > 30:
                caminos_str = ["(Demasiados caminos para mostrar. Cálculo omitido.)"]
            else:
